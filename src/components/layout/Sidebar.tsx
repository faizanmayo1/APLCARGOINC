...
const PRIMARY: Item[] = [
  ...
  { to: "/drivers", label: "Drivers", icon: UserCircle2 },
  { to: "/ai-intelligence", label: "AI Intelligence", icon: Brain },
  { to: "/exceptions", label: "Exceptions", icon: AlertTriangle, badge: 7 },
  { to: "/workflows", label: "Workflows", icon: Bot },
  { to: "/maintenance", label: "Maintenance", icon: Wrench },
];

const INTELLIGENCE: Item[] = [
  { to: "/copilot", label: "AI Copilot", icon: MessageSquareMore, ai: true },
];
  ...
  return (
    <aside className="hidden lg:flex h-screen w-[248px] shrink-0 flex-col border-r border-border-soft bg-surface/90 backdrop-blur-sm">
      ...
      <div className="relative h-14 px-5 flex items-center gap-2.5 border-b border-border-soft">
        ...
        <div className="relative h-8 w-8 rounded-lg bg-[linear-gradient(135deg,hsl(var(--brand-500)),hsl(var(--brand-700)))] grid place-items-center shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_6px_-2px_rgba(46,91,255,0.4)]">
          <span className="font-display font-semibold text-ink-invert text-[15px] leading-none">F</span>
          <span className="absolute -right-0.5 -bottom-0.5 h-2 w-2 rounded-full bg-accent-500 ring-2 ring-surface" />
        </div>
        <div className="flex flex-col leading-none">
          <span className="font-display text-[15px] font-semibold text-ink-primary tracking-tight">
            FreightIQ
          </span>
          <span className="text-[10px] uppercase tracking-[0.14em] text-ink-muted mt-0.5">
            APL Cargo Inc.
          </span>
        </div>
      </div>
        ...
        <button className="w-full flex items-center justify-between gap-2 rounded-lg px-2.5 py-2 text-left hover:bg-subtle transition-colors group">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-body-sm font-medium text-ink-primary truncate">APL Cargo Inc.</p>
              <p className="text-[11px] text-ink-muted truncate">Tier 1 · 142 trucks</p>
            </div>
          </div>
